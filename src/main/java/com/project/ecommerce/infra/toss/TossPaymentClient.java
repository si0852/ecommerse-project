...
public class TossPaymentClient {
    ...
    public TossApprovalResponse confirmPayment(PaymentInfoDto dto) {
        String authHeaderValue = "Basic " + Base64.getEncoder().encodeToString((tossSecretKey + ":").getBytes());

        try {
            TossApprovalResponse response = tossRestClient.post()
                    .uri("/confirm")
                    .header("Authorization", authHeaderValue)
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(dto)
                    .retrieve()
                    .body(TossApprovalResponse.class);

            return response;
        } catch (HttpClientErrorException e) {
            String errorBody = e.getResponseBodyAsString();
            throw BusinessException.HttpRequestConfirmError("토스 결제 승인 실패[+ "+"[" +dto.getOrderId() +"]" +": " + errorBody);
        } catch (Exception e) {
